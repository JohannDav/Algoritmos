...
// Esta funcion le dice a sort() como comparar dos elementos del arreglo
// Compara por el ID (columna 0)
bool compararPorID(const float fila1[5], const float fila2[5]) {
    return fila1[0] < fila2[0];  // Orden ascendente por ID
}
...
int ordenamientoQuick();           // Modificado para usar sort() del compilador
// void quickSort(float arreglo[][5], int limite_izq, int limite_der); // COMENTARIZADO - Usa sort() en su lugar
...
// ================= Metodo QuickSort USANDO sort() del compilador =================
// NOTA: Se comento la implementacion manual de QuickSort y se usa la funcion sort() de la libreria <algorithm>

/*
void quickSort(float arreglo[][5], int limite_izq, int limite_der) {
    // IMPLEMENTACION MANUAL DE QUICKSORT - COMENTADA PARA USAR sort() DEL COMPILADOR
    izq = limite_izq;
    der = limite_der;
...
int ordenamientoQuick() {
    ...
    // ================= USANDO LA FUNCION sort() DEL COMPILADOR =================
    // sort() requiere un puntero al inicio del arreglo y un puntero al final
    // Como tenemos una matriz, sort() necesita saber como comparar los elementos
    // Usamos la funcion de comparacion 'compararPorID' definida al inicio
    
    // Convertir el arreglo 2D a un formato que sort() pueda manejar
    // sort() espera un rango de elementos. Para una matriz, tratamos cada fila como un elemento
    
    // Forma correcta: Crear un arreglo de punteros a filas
    float* ptrArreglo[100];
    for (int idx = 0; idx < n; idx++) {
        ptrArreglo[idx] = arreglo[idx];
    }
    
    // Llamar a sort() con la funcion de comparacion personalizada
    sort(ptrArreglo, ptrArreglo + n, [](const float* a, const float* b) {
        return a[0] < b[0];  // Ordenar por ID (columna 0)
    });
...
int main() {
    ...
    for (repeticion = 0; repeticion == 0;) {
        ...
        case (12): // Metodo de ordenamiento QuickSort (USANDO sort() del compilador)
            ordenamientoQuick();
            break;
...
int menu() {
    ...
    cout << "\n12 - Metodo de ordenamiento QuickSort (funcion sort del compilador)";
