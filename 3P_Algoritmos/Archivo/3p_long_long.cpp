...
int repeticion = 0;
int numeroBuscar;
float arreglo[1000]; //Tamaño 1,000
float arreglo2[1000]; //Tamaño 1,000
...
void identificar_mayor(long long arreglo[], int i, int n);
void conformar_arbol(long long arreglo[], int n);
void ordenamientoQuick();
void quickSort(long long arreglo[], int limite_izq, int limite_der);
int identVotante();
...
int identVotante() {
	...
	for(i = 0; i < n; i++){
		if(arreglo[i] >= 18){
			cout << "\nEl votante tiene una edad de: " << arreglo [i] << " y es mayor de edad " << endl;
		}
		else{
			cout << "\nEl votante tiene una edad de: " << arreglo [i] << " y es menor de edad " << endl;
		}
	}
	return (0);
}
...
// ================= IdentVotante =================
int identInscrito() {
	cout << "\nIdentificador de alumno inscrito ";
